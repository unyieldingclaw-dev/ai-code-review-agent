...
describe('PerformanceAgent', () => {
  ...
  it('parses a valid finding and stamps agent name', async () => {
    const raw = JSON.stringify([{ severity: 'medium', basis: 'INFERRED', confidence: 70, file: 'src/api.ts', line: 22, title: 'N+1 query in loop', detail: 'Each iteration issues a separate DB query', suggestion: 'Batch the queries outside the loop' }])
