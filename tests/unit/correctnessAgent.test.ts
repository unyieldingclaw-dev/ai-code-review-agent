...
describe('CorrectnessAgent', () => {
  ...
  it('parses a valid finding and stamps agent name', async () => {
    const raw = JSON.stringify([{ severity: 'high', basis: 'VERIFIED', confidence: 85, file: 'src/utils.ts', line: 15, title: 'Off-by-one in slice', detail: 'Array slice uses wrong end index', suggestion: 'Change arr.slice(0, n-1) to arr.slice(0, n)' }])
