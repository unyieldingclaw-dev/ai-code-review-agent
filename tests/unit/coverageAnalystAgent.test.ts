...
describe('CoverageAnalystAgent', () => {
  ...
  it('parses a valid finding and stamps agent name', async () => {
    const raw = JSON.stringify([{ severity: 'medium', basis: 'INFERRED', confidence: 75, file: 'src/auth.ts', line: 20, title: 'No test for error branch', detail: 'The catch block in validateToken has no test coverage', suggestion: 'Add a test that passes an invalid token' }])
