...
describe('ErrorHandlingAgent', () => {
  ...
  it('returns empty array when provider returns empty JSON array', async () => {
    expect(
      await new ErrorHandlingAgent(makeProvider('[]'), DEFAULT_CONFIG).run({ diff: 'diff content' })
    ).toEqual([])
  })
  ...
  it('returns empty array on parse failure', async () => {
    expect(
      await new ErrorHandlingAgent(makeProvider('not json'), DEFAULT_CONFIG).run({ diff: 'diff' })
    ).toEqual([])
  })
