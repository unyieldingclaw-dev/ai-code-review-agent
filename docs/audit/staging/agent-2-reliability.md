...
**Catch-all `**`:** Becomes `._` in the regex. With the anchored branch (`hasSlash = false`), the regex `(^|/)[^/]_(/|$)` — wait, let me re-check. `**` is converted: `\*\*` → `\x00` → `.*`. With no slash in `**`, `hasSlash = false`, pattern becomes `(^|/).*(/|$)` which matches any file path. This correctly excludes all files. Safe.
