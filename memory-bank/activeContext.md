...
**v1.12.0 — deterministic false-positive filters.** Branch
`fix/deterministic-false-positive-filters`, [PR #32](https://github.com/unyieldingclaw-dev/ai-code-review-agent/pull/32),
stacked on the still-open PR #31.
...
**Verified state:** 714 unit tests · `npm audit` 0 (prod + dev) · `npm run check` green ·
...
**Not done:** PR #31 and #32 unmerged; v1.12.0 committed but **not published**; dependabot PR #14
(gitleaks 2→3, a major bump touching the pre-push secret scan) undecided.
...
- PMB reported ACR's agent-count announcement prints before policy filtering (their item 3a) —
  real, unfixed. Their other blocking items are already fixed on this branch.
...
- **714 unit tests** across 45 test files; `npm audit` clean (prod + dev)
...
- **npm**: `ai-review-agent@1.11.0` published; 1.12.0 committed on the branch, not yet published

## Next Steps

- **Land the stack**: merge PR #31, then [PR #32](https://github.com/unyieldingclaw-dev/ai-code-review-agent/pull/32)
  (auto-retargets to `main`). Pushing needs `/change-review` first — that gate is separate from the
  `/code-review` one used for committing.
- **Publish v1.12.0** after merge: tag, then `npm publish` from `main`. Not yet done.
...
- **NPM token renewal**: `github-actions-publish` expires Sep 8 2026 — create a new Automation token
  and update the `NPM_TOKEN` GitHub Actions secret before then.
- **PMB item 3a** (unfixed): the agent-count announcement is printed before `evaluatePolicy` runs, so
  a profile naming 3 agents can report `[1/2]` when policy skips one (e.g. `security` excludes `.md`,
  so an all-markdown diff drops it). Cosmetic, but it erodes trust in a gating role.
